...
describe("resolveAnnounceTarget", () => {
  ...
  it("hydrates Slack accountId from sessions.list in multi-account setups", async () => {
    const { resolveAnnounceTarget } = await loadResolveAnnounceTarget();
    callGatewayMock.mockResolvedValueOnce({
      sessions: [
        {
          key: "agent:main:slack:channel:C0123456",
          deliveryContext: {
            channel: "slack",
            to: "channel:C0123456",
            accountId: "forge",
    ...
    const target = await resolveAnnounceTarget({
      sessionKey: "agent:main:slack:channel:C0123456",
      displayKey: "agent:main:slack:channel:C0123456",
    });
    expect(target).toEqual({
      channel: "slack",
      to: "channel:C0123456",
