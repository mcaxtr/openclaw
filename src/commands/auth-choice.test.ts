...
describe("applyAuthChoice", () => {
  ...
  it("stores openai-codex OAuth with canonical profile id", async () => {
    await setupTempState();

    const profileId = expectedOpenAICodexProfileId({
    ...
    expect(await readAuthProfile(profileId)).toMatchObject({
      type: "oauth",
      provider: "openai-codex",
      refresh: "refresh-token",
      email: "user@example.com",
